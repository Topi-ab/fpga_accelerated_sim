...
public:
    ...
    void inline write_bits(size_t bit_offset, size_t bit_width, word_t data) {
        ...
        size_t src_shift = 0;  // how many bits from data we’ve already used
        ...
        for (auto idx = begin_idx; idx <= end_idx; ++idx) {
            ...
            // take the next wr_bits from data, starting at src_shift
            atomic_t v = (static_cast<atomic_t>(data) >> src_shift) & mask;
    ...
    void inline wr_hw(size_t word_address, typename shadow_t::wr_word_t data) {
        shadow_.wr_hw(word_address, data);
    }

/*
    template<typename word_t>
    word_t inline read_bits(size_t bit_offset, size_t bit_width) {
        using atomic_t = typename shadow_t::rd_word_t;
        static constexpr size_t ATOMIC_BITS = sizeof(atomic_t) * 8;

        if(bit_offset > 0)
            std::cout << "SLICER: read_bits(" << bit_offset << ", " << bit_width << ");\n";
        ...
        for(auto idx = begin_idx; idx <= end_idx; idx ++) {
            ...
            size_t bit_begin = (idx == begin_idx) ? (bit_offset % ATOMIC_BITS) : 0;
            size_t bit_end = (idx == end_idx) ? ((bit_offset + bit_width - 1) % ATOMIC_BITS) : (ATOMIC_BITS - 1);
            ...
            r |= (rd_word << (bit_begin + (idx - begin_idx) * ATOMIC_BITS));
        }
